...
const allTemplates = {
  ...
  'vite-template-redux': `npx tiged --mode=git ${gitHubUrl?.remoteUrl}/packages/vite-template-redux#${gitHubUrl?.currentBranch}${platform() === 'win32' ? '' : `#${gitHubUrl?.commitHash}`} example -v && cd example && npm install`,
}
